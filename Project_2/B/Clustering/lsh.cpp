    ...
    for(int i=0; i<query_points.size(); i++){
        ...
        for(int j=0; j<L; j++){
            ...
            for(int z=0; z<G_Hash_Tables[j][q_sum].size(); z++){
                ...
                long double d = euc_dist(temp_p.getCoord(), query_points[i].getCoord());
...
double lsh::brute_force(Point query_point, vector<Point> input_points, ofstream& out_file){
    ...
    for (int i=0; i<input_points.size(); i++){
        d = euc_dist(query_point.getCoord(), input_points[i].getCoord());
        if(d<min_dist){
            min_dist = d;
            ...
            for(int z=0; z<G_Hash_Tables[j][q_sum].size(); z++){
                ...
                if ( (query_points[i].get_ID(j)) == temp_p.get_ID(j) ) {
                    ...
                    */
                    ...
                    double d = euc_dist(temp_p.getCoord(), query_points[i].getCoord());
...
void lsh::query_range_search(vector<Point> query_points, vector<Point> input_points, double R, ofstream& out_file){
    ...
    for(int i=0; i<query_points.size(); i++){
        ...
        {          
            N_near temp_near = list_N[t];
            out_file<<"Neighbor "<<t<<" id: "<<temp_near.p.getID()<<endl;
        }
...
Apotelesma_range lsh::query_range_search_clu_DFD(Point query_points,double R){
    long long int q_sum;
    double min_dist;
    min_dist = DBL_MAX;
    int id;
    ...
    Apotelesma_range ap;
        ...
        for(int j=0; j<L; j++){
            ...
            for(int z=0; z<G_Hash_Tables[j][q_sum].size(); z++){
                ...
                //double d = euc_dist(temp_p.getCoord(), query_points.getCoord());
                double d = DiscreteFrechet(temp_p.getCurve(), query_points.getCurve());
